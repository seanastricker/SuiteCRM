...
class ParentCommunicationHelper
{
    ...
    public static function sendEmail($to, $subject, $message, $recipient_name) {
        try {
            // Get SMTP settings from Administration
            require_once('data/BeanFactory.php');
            require_once('include/SugarPHPMailer.php');
            
            $admin = BeanFactory::newBean('Administration');
            if (!$admin) {
                error_log("Could not load Administration bean");
                return false;
            }
            ...
            if (empty($admin->settings['mail_smtpserver'])) {
                error_log("SMTP not configured");
                return false;
            }
            
            // Create SugarPHPMailer instance
            $mail = new SugarPHPMailer();
            
            // Configure SMTP
            $mail->isSMTP();
            $mail->Host = $admin->settings['mail_smtpserver'];
            $mail->Port = !empty($admin->settings['mail_smtpport']) ? $admin->settings['mail_smtpport'] : 587;
            
            // Convert numeric SSL setting to string (SuiteCRM stores as numbers: 0=none, 1=ssl, 2=tls)
            $ssl_setting = $admin->settings['mail_smtpssl'] ?? '2';
            switch ($ssl_setting) {
                case '1':
                case 1:
                    $mail->SMTPSecure = 'ssl';
            ...
            // Authentication
            if (!empty($admin->settings['mail_smtpuser'])) {
                $mail->SMTPAuth = true;
            ...
            $from_email = !empty($admin->settings['mail_smtpuser']) ? $admin->settings['mail_smtpuser'] : 'noreply@sportsleague.com';
